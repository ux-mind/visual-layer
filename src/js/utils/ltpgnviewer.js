...
var AutoPlayInterval,
	...
	Delay = 1000,
	BoardClicked = -1,
	isCapturedPieces = false,
	CandidateStyle = '';
var PieceName = 'KQRBNP',
	ShowPieceName = 'KQRBNP';
...
function InitImages() {
	...
	for (ii = 0; ii < 2; ii++) {
		PiecePic[ii][0] = new Image();
		PiecePic[ii][0].src = ImagePath + ColorName[ii] + 'k.gif';
		PiecePic[ii][1] = new Image();
		PiecePic[ii][1].src = ImagePath + ColorName[ii] + 'q.gif';
		PiecePic[ii][2] = new Image();
		PiecePic[ii][2].src = ImagePath + ColorName[ii] + 'r.gif';
		PiecePic[ii][3] = new Image();
		PiecePic[ii][3].src = ImagePath + ColorName[ii] + 'b.gif';
		PiecePic[ii][4] = new Image();
		PiecePic[ii][4].src = ImagePath + ColorName[ii] + 'n.gif';
		PiecePic[ii][5] = new Image();
		PiecePic[ii][5].src = ImagePath + ColorName[ii] + 'p.gif';
	}
	LabelPic[0] = new Image();
	LabelPic[0].src = ImagePath + '8_1.gif';
	LabelPic[1] = new Image();
	LabelPic[1].src = ImagePath + 'a_h.gif';
	LabelPic[2] = new Image();
	LabelPic[2].src = ImagePath + '1_8.gif';
	LabelPic[3] = new Image();
	LabelPic[3].src = ImagePath + 'h_a.gif';
	LabelPic[4] = new Image();
	LabelPic[4].src = ImagePath + '1x1.gif';
...
function OpenUrl(ss) {
	if (ss != '') parent.frames[1].location.href = ss;
	else {
...
function Init(rr) {
	...
	if (document.BoardForm && document.BoardForm.FEN) document.BoardForm.FEN.value = FenString;
	if (FenString == StandardFen) {
		...
		for (ii = 0; ii < 2; ii++) {
			for (jj = 0; jj < 2; jj++) Castling[ii][jj] = 1;
		}
		EnPass = -1;
		HalfMove[0] = 0;
		if (document.BoardForm) {
			RefreshBoard();
			if (document.BoardForm.Position) document.BoardForm.Position.value = '';
		...
		while (cc != ' ') {
			cc = cc.charCodeAt(0);
			if (cc == PieceName.toUpperCase().charCodeAt(0)) Castling[0][0] = 1;
			if (cc == PieceName.toUpperCase().charCodeAt(1)) Castling[0][1] = 1;
			if (cc == PieceName.toLowerCase().charCodeAt(0)) Castling[1][0] = 1;
			if (cc == PieceName.toLowerCase().charCodeAt(1)) Castling[1][1] = 1;
			if (cc >= 65 && cc <= 72) {
				//A...H  for Chess960
...
function MoveBack(nn) {
	...
	if (HistCommand[MoveCount - StartMove])
		NewCommands = HistCommand[MoveCount - StartMove].split('|');
	...
	if (document.BoardForm) {
		...
		if (document.BoardForm.Position) {
			if (MoveCount > StartMove)
				document.BoardForm.Position.value = TransformSAN(
					HistMove[MoveCount - StartMove - 1]
				);
			else document.BoardForm.Position.value = '';
		}
	}
	if (TargetDocument) HighlightMove('m' + MoveCount + 'v' + CurVar);
		...
		if (ffst >= 0) {
			...
			if (mmove0 != '') {
				if (ParseMove(mmove0, true) > 0) {
					mmove1 = mmove0;
					if (MoveType == 0)
						HistMove[MoveCount - StartMove] =
							Math.floor((MoveCount + 2) / 2) + '.' + mmove1;
					else
						HistMove[MoveCount - StartMove] =
							Math.floor((MoveCount + 2) / 2) + '. ... ' + mmove1;
...
function ParseMove(mm, sstore) {
	...
	if (MoveCount > StartMove) {
		...
		if (
			HistType[0][MoveCount - StartMove - 1] == 5 &&
			Math.abs(HistPosY[0][MoveCount - StartMove - 1] - PiecePosY[1 - MoveType][ii]) == 2
		)
	...
	if (xx1 < 0 || xx1 > 7 || yy1 < 0 || yy1 > 7) {
		if (mm.indexOf('O') >= 0 || mm.indexOf('0') >= 0) {
			if (
				...
				if (EvalMove(ttype0, 6, xx0, yy0, ttype1, xx1, yy1, ccapt, sstore)) return 1;
				return 0;
			}
			if (
				mm.indexOf('O-O') >= 0 ||
				...
				if (EvalMove(ttype0, 7, xx0, yy0, ttype1, xx1, yy1, ccapt, sstore)) return 1;
				return 0;
			}
			return 0;
		}
		if (mm.indexOf('---') >= 0 || mm.indexOf('���') >= 0) {
			...
			if (EvalMove(ttype0, 8, xx0, yy0, ttype1, xx1, yy1, ccapt, sstore)) return 1;
			return 0;
		}
		return 0;
	}
	...
	if (ll > 0) {
		...
		if (yy0 >= 0 && isNaN(mm.charAt(ll - 2 - ccapt))) {
			//Smith Notation
			...
			if (xx0 < 0 || xx0 > 7) xx0 = -1;
				...
				if (ttype0 == 0 && xx0 - xx1 > 1 && yy0 == yy1) {
					if (EvalMove(ttype0, 6, xx0, yy0, -1, -1, -1, 0, sstore)) return 1;
					return 0;
				}
				if (ttype0 == 0 && xx1 - xx0 > 1 && yy0 == yy1) {
					if (EvalMove(ttype0, 7, xx0, yy0, -1, -1, -1, 0, sstore)) return 1;
					return 0;
				}
			}
		}
	...
	if (sstore) {
		for (ii = 0; ii < 16; ii++) {
			if (PieceType[MoveType][ii] == ttype0) {
				if (EvalMove(ii, ttype0, xx0, yy0, ttype1, xx1, yy1, ccapt, true)) return 1;
			}
		}
	} else {
		ll = 0;
		for (ii = 0; ii < 16; ii++) {
			if (PieceType[MoveType][ii] == ttype0) {
				if (EvalMove(ii, ttype0, xx0, yy0, ttype1, xx1, yy1, ccapt, false)) ll++;
			}
		}
		return ll;
	}
...
function CanCastleLong() {
	...
	while (jj < 16) {
		if (
			PiecePosX[MoveType][jj] < PiecePosX[MoveType][0] &&
...
function EvalMove(ii, ttype0, xx0, yy0, ttype1, xx1, yy1, ccapt, sstore) {
	...
	if (ttype0 == 6) {
		...
		if (StoreMove(0, 0, 2, MoveType * 7, jj, 2, 3, MoveType * 7, sstore)) return true;
		else return false;
	}
	if (ttype0 == 7) {
		...
		jj = CanCastleShort();
		if (jj < 0) return false;
		if (StoreMove(0, 0, 6, MoveType * 7, jj, 2, 5, MoveType * 7, sstore)) return true;
		return false;
	}
	if (ttype0 == 8) {
		// --- NullMove
	...
	if (PiecePosX[MoveType][ii] == xx1 && PiecePosY[MoveType][ii] == yy1) return false;
	if (ccapt == 0 && Board[xx1][yy1] != 0) return false;
	if (ccapt > 0 && sign(Board[xx1][yy1]) != 2 * MoveType - 1) {
		if (ttype0 != 5 || CanPass != xx1 || yy1 != 5 - 3 * MoveType) return false;
	}
	...
	if (ttype0 == 1) {
		if (
			Math.abs(PiecePosX[MoveType][ii] - xx1) != Math.abs(PiecePosY[MoveType][ii] - yy1) &&
			(PiecePosX[MoveType][ii] - xx1) * (PiecePosY[MoveType][ii] - yy1) != 0
		)
			return false;
	}
	if (ttype0 == 2) {
		if ((PiecePosX[MoveType][ii] - xx1) * (PiecePosY[MoveType][ii] - yy1) != 0) return false;
	}
	if (ttype0 == 3) {
	...
	if (StoreMove(ii, ttype1, xx1, yy1, jj, ttype3, xx3, yy3, sstore)) return true;
	return false;
}
...
function StoreMove(ii, ttype1, xx1, yy1, jj, ttype3, xx3, yy3, sstore) {
	var iis_check = 0,
		ll,
		cc = MoveCount - StartMove,
		ff = PiecePosX[MoveType][0],
		dd = 0;
	//  if ((ttype1==5)||((jj>=0)&&(ttype3<0)))
	if (PieceType[MoveType][ii] == 5 || (jj >= 0 && ttype3 < 0)) HalfMove[cc + 1] = 0;
	else HalfMove[cc + 1] = HalfMove[cc] + 1;
	HistPiece[0][cc] = ii;
	...
	Board[PiecePosX[MoveType][ii]][PiecePosY[MoveType][ii]] = 0;
	if (jj >= 0) {
		if (ttype3 < 0) Board[PiecePosX[1 - MoveType][jj]][PiecePosY[1 - MoveType][jj]] = 0;
		else Board[PiecePosX[MoveType][jj]][PiecePosY[MoveType][jj]] = 0;
	}
	...
	if (jj >= 0) {
		if (ttype3 < 0) Board[PiecePosX[1 - MoveType][jj]][PiecePosY[1 - MoveType][jj]] = 0;
			...
			Board[PiecePosX[MoveType][jj]][PiecePosY[MoveType][jj]] =
				(PieceType[MoveType][jj] + 1) * (1 - 2 * MoveType);
	}
	Board[PiecePosX[MoveType][ii]][PiecePosY[MoveType][ii]] =
		(PieceType[MoveType][ii] + 1) * (1 - 2 * MoveType);
	...
	if (iis_check == 0 && sstore) {
		MoveArray[cc] =
			String.fromCharCode(97 + HistPosX[0][cc]) +
			(HistPosY[0][cc] + 1) +
			String.fromCharCode(97 + PiecePosX[MoveType][ii]) +
			(PiecePosY[MoveType][ii] + 1);
...
function ExecCommands(nnc, hh) {
	...
	for (ii = 0; ii < NewCommands.length; ii++) {
		...
		if (tt == 'this' || tt == 'last') {
			if (tt == 'this') {
				kk = MoveCount - StartMove - 1;
				ll = 0;
			} else {
				kk = MoveCount - StartMove - 2;
				ll = 1;
			}
			if (kk >= 0) {
				...
				if (tt == 'B' || tt == 'C') {
					NewCommands[nn] =
						tt + String.fromCharCode(97 + HistPosX[0][kk]) + (1 + HistPosY[0][kk]) + cc;
					NewCommands[nn + 1] =
						tt +
						String.fromCharCode(
							97 + PiecePosX[(MoveType + ll + 1) % 2][HistPiece[0][kk]]
						) +
						(1 + PiecePosY[(MoveType + ll + 1) % 2][HistPiece[0][kk]]) +
						cc;
				}
				if (tt == 'A') {
					NewCommands[nn] =
						tt + String.fromCharCode(97 + HistPosX[0][kk]) + (1 + HistPosY[0][kk]);
					NewCommands[nn] +=
						String.fromCharCode(
							97 + PiecePosX[(MoveType + ll + 1) % 2][HistPiece[0][kk]]
						) +
			...
			if (tt == 'B' || tt == 'C') {
				nn =
					NewCommands[ii].charCodeAt(1) -
					97 +
					(8 - parseInt(NewCommands[ii].charAt(2))) * 8;
				if (nn >= 0 && nn <= 63) {
					if (isRotated) nn = 63 - nn;
...
function RefreshBoard(rr) {
	...
	if (isSetupBoard) {
		if (isRotated) {
			for (ii = 0; ii < 8; ii++) {
				for (jj = 0; jj < 8; jj++) {
					if (Board[ii][jj] == 0) SetImg(63 - ii - (7 - jj) * 8, BoardPic);
					else
						SetImg(
							63 - ii - (7 - jj) * 8,
							PiecePic[(1 - sign(Board[ii][jj])) / 2][Math.abs(Board[ii][jj]) - 1]
						);
				}
			}
		} else {
			for (ii = 0; ii < 8; ii++) {
				for (jj = 0; jj < 8; jj++) {
					if (Board[ii][jj] == 0) SetImg(ii + (7 - jj) * 8, BoardPic);
					else
						SetImg(
							ii + (7 - jj) * 8,
							PiecePic[(1 - sign(Board[ii][jj])) / 2][Math.abs(Board[ii][jj]) - 1]
		...
		for (ii = 0; ii < 8; ii++) {
			for (jj = 0; jj < 8; jj++) {
				if (Board[ii][jj] == 0) {
					if (isRotated) SetImg(63 - ii - (7 - jj) * 8, BoardPic);
					else SetImg(ii + (7 - jj) * 8, BoardPic);
				}
			}
		}
		for (ii = 0; ii < 2; ii++) {
			for (jj = 0; jj < 16; jj++) {
				if (PieceType[ii][jj] >= 0) {
					...
					if (isRotated) SetImg(63 - kk, PiecePic[ii][PieceType[ii][jj]]);
					else SetImg(kk, PiecePic[ii][PieceType[ii][jj]]);
				}
			}
		}
		if (isCapturedPieces) {
			...
			if (mm < kk) mm = kk;
			while (kk < 4) {
				SetImg(64 + kk0 + (kk - (kk % 4)) / 4 + (kk % 4) * 4, BoardPic);
				kk++;
			}
			while (kk < 16) {
				SetImg(64 + kk0 + (kk - (kk % 4)) / 4 + (kk % 4) * 4, LabelPic[4]);
				kk++;
			}
			...
			if (mm < kk) mm = kk;
			while (kk < 4) {
				SetImg(92 + kk0 + (kk - (kk % 4)) / 4 - (kk % 4) * 4, BoardPic);
				kk++;
			}
			while (kk < 16) {
				SetImg(92 + kk0 + (kk - (kk % 4)) / 4 - (kk % 4) * 4, LabelPic[4]);
				kk++;
			}
			mm = Math.ceil(mm / 4);
...
function HighlightCandidates(nn, ccs) {
						...
						if (bb == 0) {
							if (MoveCount > StartMove) {
								...
								if (
									HistType[0][MoveCount - StartMove - 1] == 5 &&
									Math.abs(
										HistPosY[0][MoveCount - StartMove - 1] -
											PiecePosY[1 - MoveType][dd]
									) == 2
								)
							...
							if (CanPass == ii0 + ddi) {
								...
								if (!IsCheck(PiecePosX[tt][0], PiecePosY[tt][0], tt))
									aa[nna++] =
										String.fromCharCode(ii0 + ddi + 97) + (jj0 + cc + 1);
								Board[ii0 + ddi][jj0 + cc] = bb;
							}
						}
					}
	...
	if (nna > 0 && ccs != ' ') {
		...
		if (tt == 'A') {
			...
			for (jj = 1; jj < nna; jj++)
				bb += ',' + tt + String.fromCharCode(ii0 + 97) + (jj0 + 1) + aa[jj] + cc;
		} else {
			bb = tt + aa[0] + cc;
...
function SetBoardClicked(nn) {
	...
	if (!document.images[ImageOffset].style) {
		BoardClicked = nn;
		return;
	}
	...
	if (BoardClicked >= 0) {
		if (BoardClicked < 64) {
			...
			else document.images[ImageOffset + BoardClicked].style.borderColor = BorderColor;
	...
	if (BoardClicked >= 0) {
		if (BoardClicked < 64) {
			...
			else document.images[ImageOffset + BoardClicked].style.borderColor = '#FF0000';
		...
		for (jj0 = 0; jj0 < 8; jj0++) {
			if (sign(Board[ii0][jj0]) == ((MoveCount + 1) % 2) * 2 - 1) {
				if (Math.abs(Board[ii0][jj0]) == 6) {
					...
					mm =
						PieceName.charAt(Math.abs(Board[ii0][jj0]) - 1) +
						String.fromCharCode(ii0 + 97) +
						eval(jj0 + 1);
...
function BoardClick(nn, bb) {
	...
	if (BoardClicked < 0) {
		...
		if (sign(Board[ii0][jj0]) != ((MoveCount + 1) % 2) * 2 - 1) {
			...
			if (ffst >= 0) ssub = ShortPgnMoveText[0][CurVar].substring(0, ffst);
			else ssub = ShortPgnMoveText[0][CurVar];
			if (ParseMove(mm, false) == 0) {
				SetBoardClicked(-1);
				return;
			}
			if (!isNullMove) return;
			if (MoveCount % 2 == 0) {
				if (!confirm('White nullmove?')) return;
			} else {
				if (!confirm('Black nullmove?')) return;
			}
			for (vv = CurVar; vv < ShortPgnMoveText[0].length; vv++) {
				if (
					vv == CurVar ||
					(ShortPgnMoveText[1][vv] == CurVar && ShortPgnMoveText[2][vv] == MoveCount)
				) {
					...
					if (ffst >= 0) {
						...
						if (mmove0.indexOf(mm) == 0) {
							...
							if (window.UserMove)
								setTimeout("UserMove(1,'" + mmove0 + "')", Delay / 2);
						}
					}
				}
			}
			if (vv < ShortPgnMoveText[0].length + 1) {
				...
				MoveType = 1 - MoveType;
				if (document.BoardForm) {
					if (document.BoardForm.PgnMoveText)
						document.BoardForm.PgnMoveText.value = ssub + mm + ' ';
					if (document.BoardForm.Position)
						document.BoardForm.Position.value = TransformSAN(
							HistMove[MoveCount - StartMove - 1]
						);
	...
	if (Math.abs(Board[ii0][jj0]) == 1) {
		if (PiecePosY[MoveType][0] == jj1) {
			...
			if (Board[ii1][jj1] == (1 - 2 * MoveType) * 3) {
				//for Chess960
				if (ii1 > ii0) mm = 'O-O';
				if (ii1 < ii0) mm = 'O-O-O';
			}
	...
	if (
		...
		while (pp == 0) {
			if (pp == 0) {
				if (confirm('Queen ' + PieceName.charAt(1) + ' ?')) pp = 1;
			}
			if (pp == 0) {
				if (confirm('Rook ' + PieceName.charAt(2) + ' ?')) pp = 2;
			}
			if (pp == 0) {
				if (confirm('Bishop ' + PieceName.charAt(3) + ' ?')) pp = 3;
			}
			if (pp == 0) {
				if (confirm('Knight ' + PieceName.charAt(4) + ' ?')) pp = 4;
			}
		}
	...
	if (Math.abs(Board[ii0][jj0]) != 1) {
		...
		if (Math.abs(Board[ii0][jj0]) == 6) {
			...
			mmm =
				mm.substr(0, 1) + String.fromCharCode(ii0 + 97) + eval(jj0 + 1) + mm.substr(1, 11);
...
function RotateBoard(bb) {
	...
	ExecCommands();
	isRotated = bb;
	if (document.BoardForm && document.BoardForm.Rotated) document.BoardForm.Rotated.checked = bb;
	RefreshBoard();
...
function IsMate() {
	var aa,
		...
		jj0,
		nn = 0,
		ii = IsCheck(PiecePosX[MoveType][0], PiecePosY[MoveType][0], MoveType);
...
function ShowSAN(ss) {
	...
	if (document.BoardForm && document.BoardForm.PgnMoveText) {
		...
		var ww = window.open(
			'',
			'',
			'width=600, height=400, menubar=no, locationbar=no, resizable=yes, status=no, scrollbars=yes'
		);
		ww.document.open();
...
function ApplyFEN(ss) {
	if (ss.length == 0) {
		...
		FenString = ss;
		if (document.BoardForm && document.BoardForm.FEN) document.BoardForm.FEN.value = ss;
	}
}
	...
	for (jj = 7; jj >= 0; jj--) {
		...
		for (ii = 0; ii < 8; ii++) {
			if (Board[ii][jj] == 0) ee++;
				...
				if (Board[ii][jj] > 0) ss = ss + PieceName.toUpperCase().charAt(Board[ii][jj] - 1);
				else ss = ss + PieceName.toLowerCase().charAt(-Board[ii][jj] - 1);
			}
		}
		if (ee > 0) ss = ss + '' + ee;
		if (jj > 0) ss = ss + '/';
...
function MakePuzzle() {
	...
	if (document.BoardForm.EmailBlog) {
		if (document.BoardForm.EmailBlog.checked) pp = ScriptPath;
	}
...
function ApplyPgnMoveText(ss, rroot, ddocument, ggame) {
	...
	if (!isNaN(rroot)) {
		...
		while (
			ii < uuc.length - 1 &&
			((ii > 0 && uuc.charAt(ii - 1) != ' ') || isNaN(parseInt(uuc.charAt(ii))))
		)
			ii++;
		while (ii < uuc.length - 1 && !isNaN(parseInt(uuc.charAt(ii)))) {
			bb = 10 * bb + parseInt(uuc.charAt(ii));
			ii++;
		}
		if (ii < uuc.length - 1) {
			uuu = uuc.substr(ii, 3);
...
function GetHTMLMoveText(vvariant, nnoscript, ccommenttype, sscoresheet) {
			...
			if (sstart >= 0) {
				if (sscoresheet) {
					...
					if (ss == '') {
						...
						if (MoveCount % 2 == 1)
							ss +=
								"<table width='100%' cellpadding=0 cellspacing=0><colgroup><col width='20%'><col width='40%'><col width='40%'></colgroup><tr><th>" +
								eval((MoveCount + 1) / 2) +
								'.</th>';
						else
							ss +=
						...
						if (MoveCount % 2 == 1)
							ss += '<tr><th>' + eval((MoveCount + 1) / 2) + '.</th>';
					}
					ss += '<th>';
				} else ss += tt.substr(0, sstart);
				if (!nnoscript)
					ss +=
						'<a href="javascript:SetMove{{' +
						MoveCount +
						',' +
						vv +
						gg +
				...
				if (sscoresheet) {
					...
					if (sscoresheet == 2) {
						if (MoveCount % 80 == 0)
							ss +=
								"</table></td></tr></table><table width='100%' cellpadding=0 cellspacing=0><tr><td width='50%'><table width='100%' cellpadding=0 cellspacing=0><colgroup><col width='20%'><col width='40%'><col width='40%'></colgroup>";
						else {
							if (MoveCount % 40 == 0)
								ss +=
									"</table></td><td width='50%'><table width='100%' cellpadding=0 cellspacing=0><colgroup><col width='20%'><col width='40%'><col width='40%'></colgroup>";
						}
					}
				}
			} else ffst = -1;
	...
	if (!vvariant) {
		...
		if (ccommenttype >= 1) {
			...
			uu = tt[0];
			for (ii = 1; ii < ll; ii++)
				uu += '<a href="javascript:SetMove()"><span title="' + tt[ii];
			tt = uu.split('}');
			ll = tt.length;
			uu = tt[0];
			for (ii = 1; ii < ll; ii++)
				uu +=
					"\" onMouseDown=\"if (this.innerHTML=='{'+this.title+'}') this.innerHTML='<I>{...}</I>'; else this.innerHTML='{'+this.title+'}';\"><I>{...}</I></span></a>" +
					tt[ii];
		}
	}
	isCalculating = false;
...
function HighlightMove(nn) {
	var ii,
		...
		bb,
		jj = 0,
		ll = TargetDocument.anchors.length;
	if (ll == 0) return;
	...
	for (ii = jj; ii < ll && activeAnchor < 0; ii++) {
		if (TargetDocument.anchors[ii].name == nn) {
			...
			if (
				...
				parent.document != TargetDocument &&
				TargetDocument.anchors[activeAnchor].scrollIntoView
			) {
...
function UpdateAnnotation(bb) {
	...
	if (bb) {
		const document = parent.frames['annotation'].document;

		document.open();
		document.writeln('<html><head></head><body><form>');
		document.writeln("<input type='hidden' name='MoveCount' value='" + MoveCount + "'>");
		document.write("<textarea rows=8 style='width:100%' name='Annotation'>");

		if (Annotation[MoveCount]) {
			document.write(Annotation[MoveCount]);
		}
		...
		else parent.frames['annotation'].document.forms[0].Annotation.value = '';
	}
}
...
function SaveAnnotation(ff) {
	...
	if (AnnotationFile && parent.frames['annotation'])
		parent.frames['annotation'].location.replace(
			AnnotationFile + '?MoveCount=' + mm + '&Annotation=' + escape(Annotation[mm])
		);
}
...
function GetDiagram(pp, ssp) {
	var ii,
		...
		nn,
		mm,
		ss = String.fromCharCode(13) + '<P align=center>',
		oo,
		aa = new Array(64);
	var bb = Border;
	var iip = ImagePath;
			...
			for (ii = 7; ii >= 0; ii--) {
				if (Board[ii][jj] == 0 || (pp && (Board[ii][jj] + 6) % 6 != 0))
					...
					ss +=
						...
						".gif'><IMG SRC='" +
						iip +
						ColorName[(1 - sign(Board[ii][jj])) / 2] +
						tt[Math.abs(Board[ii][jj]) - 1] +
						".gif'";
	...
	if (!document.layers) {
		var xx0,
			...
			bb1,
			kk,
			dd = parseInt(document.getElementById('Board').offsetHeight);
		if (iip != 0) {
			dd = 0;
			for (ii = 0; ii < iip.length; ii++) {
				if (!isNaN(iip.charAt(ii))) {
					dd *= 10;
					dd += parseInt(iip.charAt(ii));
				}
			}
			if (dd > 0) dd += 2 * bb;
			dd *= 8;
...
function GetCapturedPieces(iip, bb) {
	...
	while (kk < 4) {
		rr[kk % 4] += "<td><IMG SRC='" + iip + "t.gif'></td>";
		kk++;
	}
	while (kk < 16) {
		rr[kk % 4] += "<td><IMG SRC='" + iip + "1x1.gif'></td>";
		kk++;
	}
	var pp1 = new Array(0, 1, 1, 2, 2, 2, 8);
	kk = 0;
	...
	while (kk < 4) {
		rr[7 - (kk % 4)] += "<td><IMG SRC='" + iip + "t.gif'></td>";
		kk++;
	}
	while (kk < 16) {
		rr[7 - (kk % 4)] += "<td><IMG SRC='" + iip + "1x1.gif'></td>";
		kk++;
	}
...
function GetArrow(theX0, theY0, theX1, theY1, theColor) {
	...
	if (ww == 0 || hh == 0)
		dd +=
			"<div style='position:absolute;left:16px;top:16px;width:" +
			eval(ww + 2) +
			'px;height:' +
			eval(hh + 2) +
			'px;background-color:' +
			...
			while (ccr < ww) {
				...
				if (ddir)
					dd +=
						...
						'px;width:' +
						eval(ccr - ccl + 2) +
						'px;height:2px;background-color:' +
						theColor +
						";font-size:1pt;line-height:1pt;'></div>";
				else
					dd +=
...
function SwitchSetupBoard() {
	...
	cc = 0;
	if (Board[4][0] == 1 && Board[7][0] == 3) {
		ss += 'K';
		cc++;
	}
	if (Board[4][0] == 1 && Board[0][0] == 3) {
		ss += 'Q';
		cc++;
	}
	if (Board[4][7] == -1 && Board[7][7] == -3) {
		ss += 'k';
		cc++;
	}
	if (Board[4][7] == -1 && Board[0][7] == -3) {
		ss += 'q';
		cc++;
	}
	if (cc == 0) ss = ss + '-';
...
function ParsePgn(nn, gg, ffile) {
	...
	for (ii = 0; ii < tt.length; ii++) {
		kk = tt[ii][0].split(' ')[0];
		sstype[kk] = kk;
		if (tt[ii][0].substr(0, 6) == 'Black ') bblack[jj] = tt[ii][0].substr(6, tt[ii][0].length);
		if (tt[ii][0].substr(0, 6) == 'White ') wwhite[jj] = tt[ii][0].substr(6, tt[ii][0].length);
		if (tt[ii][0].substr(0, 7) == 'Result ')
			rresult[jj] = tt[ii][0].substr(7, tt[ii][0].length);
		if (tt[ii][0].substr(0, 4) == 'FEN ') ffenText[jj] = tt[ii][0].substr(4, tt[ii][0].length);
		if (tt[ii][0].substr(0, 4) == 'SAN ') ssanText[jj] = tt[ii][0].substr(4, tt[ii][0].length);
		...
		if (kk < tt[ii][1].length) {
			...
			kk = 0;
			ff = String.fromCharCode(13);
			while ((kk = ppgnText[jj].indexOf(ff, kk)) > 0)
				ppgnText[jj] = ppgnText[jj].substr(0, kk) + '' + ppgnText[jj].substr(kk + 1);
			kk = 0;
			ff = String.fromCharCode(10) + String.fromCharCode(10);
			while ((kk = ppgnText[jj].indexOf(ff, kk)) > 0)
				ppgnText[jj] =
					ppgnText[jj].substr(0, kk) + ' <br /><br /> ' + ppgnText[jj].substr(kk + 2);
			kk = 0;
			ff = String.fromCharCode(10);
			while ((kk = ppgnText[jj].indexOf(ff, kk)) > 0)
				ppgnText[jj] = ppgnText[jj].substr(0, kk) + ' ' + ppgnText[jj].substr(kk + 1);
	...
	if (document.BoardForm && document.BoardForm.ScoreSheet)
		ssh = parseInt(
			document.BoardForm.ScoreSheet.options[
				document.BoardForm.ScoreSheet.options.selectedIndex
			].value
		);
	...
	dd.writeln(
		'body { background-color:' +
			bb +
			';color:#000000;font-size:10pt;line-height:12pt;font-family:Verdana; }'
	);
	if (ssh || ParseType > 2) {
		dd.writeln('table { border-left:1px solid #808080; border-top:1px solid #808080; }');
		dd.writeln(
			'td, th { border-right:1px solid #808080; border-bottom:1px solid #808080; font-size:10pt;line-height:12pt;font-family:Verdana; vertical-align:top}'
		);
	}
	...
	if (document.BoardForm && document.BoardForm.ImagePath)
		ii =
			document.BoardForm.ImagePath.options[document.BoardForm.ImagePath.options.selectedIndex]
				.value;
	if (ii != '') ii = '&SetImagePath=' + ii;
	if (BGColor != '') ii = ii + '&SetBGColor=' + BGColor.substr(1, 6);
	if (
		document.BoardForm &&
		document.BoardForm.Border &&
		document.BoardForm.Border.options.selectedIndex > 0
	)
		ii = ii + '&SetBorder=1';
	if (parent.frames['annotation'])
		dd.writeln(
			"if (! parent.frames[0]) location.href='pgnannotator.html?'+location.href+'&SetAnnotation=" +
				AnnotationFile +
				ii +
				"';"
		);
		...
		dd.writeln(
			"if (! parent.frames[0]) location.href='ltpgnviewer.html?'+location.href+'" + ii + "';"
		);
	...
	dd.writeln(
		'        //parent.frames[0].SetPgnMoveText(' +
			uu +
			'(PgnMoveText[nn])); //variants not possible'
	);
	dd.writeln(
		'        parent.frames[0].ApplyPgnMoveText(' +
			uu +
			"(PgnMoveText[nn]),'#CCCCCC',window.document); //variants possible"
	);
	dd.writeln(
		"        //document.getElementById('GameText').innerHTML=" +
			uu +
			"(GameText[nn])+'<br />'+PgnMoveText[nn]; //pgn without html links"
	);
	if (ssh) {
		dd.writeln(
			"        if (document.getElementById) document.getElementById('GameText').innerHTML=parent.frames[0].ScoreSheetHeader(" +
				uu +
				'(GameText[nn]))+parent.frames[0].GetHTMLMoveText(0,false,true,' +
				ssh +
				')+parent.frames[0].ScoreSheetFooter(); //pgn with html links'
		);
		dd.writeln(
			'        else if (document.GameTextLayer) { with(document.GameTextLayer) { document.open(); document.write(parent.frames[0].ScoreSheetHeader(' +
				uu +
				'(GameText[nn]))+parent.frames[0].GetHTMLMoveText(0,false,true,' +
				ssh +
				')+parent.frames[0].ScoreSheetFooter()); document.close(); }}//pgn with html links'
		);
		if (parent.frames['annotation'])
			dd.writeln('        parent.frames[0].UpdateAnnotation(true);');
	} else {
		dd.writeln(
			"        if (document.getElementById) document.getElementById('GameText').innerHTML=" +
				uu +
				"(GameText[nn])+'<br />'+parent.frames[0].GetHTMLMoveText(0,false,true); //pgn with html links"
		);
		dd.writeln(
			'        else if (document.GameTextLayer) { with(document.GameTextLayer) { document.open(); document.write(' +
				uu +
				"(GameText[nn])+'<br />'+parent.frames[0].GetHTMLMoveText(0,false,true)); document.close(); }}//pgn with html links"
		);
		dd.writeln(
			'        if ((document.forms[0])&&(document.forms[0].GameList)) document.forms[0].GameList.options.selectedIndex=parseInt(nn)+1;'
		);
	}
	dd.writeln('      }');
	if (isDragDrop)
		dd.writeln('      if (parent.frames[0].SetDragDrop) parent.frames[0].SetDragDrop(1);');
	dd.writeln('      return;');
	dd.writeln('    }');
	dd.writeln('  }');
	...
	dd.writeln('}');
	dd.writeln(
		'function SetMove(mm,vv){ if (parent.frames[0].SetMove) parent.frames[0].SetMove(mm,vv); }'
	);
	if (jj > 1) {
		...
		dd.writeln(
			'  if ((kk==37)||(kk==52)||(kk==65460)) { if (parent.frames[0].MoveBack) parent.frames[0].MoveBack(1); }'
		);
		dd.writeln(
			'  if ((kk==39)||(kk==54)||(kk==65462)) { if (parent.frames[0].MoveForward) parent.frames[0].MoveForward(1); }'
		);
		dd.writeln('}');
	}
	dd.writeln('</' + 'script>');
	if (jj == 1) dd.writeln('</head><body onLoad="setTimeout(\'OpenGame(0)\',400)">');
		...
		if (ParseType < 3) {
			if (parseInt(gg) < jj)
				dd.writeln('</head><body onLoad="setTimeout(\'OpenGame(' + gg + ')\',400)">');
			else dd.writeln('</head><body>');
			dd.writeln(
				"<FORM onSubmit='return SearchGame()'><NOBR><SELECT name='GameList' onChange='OpenGame(this.options[selectedIndex].value)' SIZE=1>"
			);
			...
			if (jj < 24) dd.writeln('<!--');
			dd.writeln(
				"<INPUT name='SearchText' size=12><select name='SearchType'><option>Player</option>"
			);
			...
			for (ii = 0; ii < jj; ii++) {
				...
				if (ccT.length == 3 && ccT.indexOf('-') == 1) {
					...
					ccCT[ccI[wwhite[ii]]][ccI[bblack[ii]]] +=
						"&nbsp;<a href='javascript:OpenGame(" +
						ii +
						")'>" +
						ccT.substr(0, 1) +
						'</a>&nbsp;';
					ccCT[ccI[bblack[ii]]][ccI[wwhite[ii]]] +=
						"&nbsp;<a href='javascript:OpenGame(" +
						ii +
						")'>" +
						ccT.substr(2, 1) +
					...
					ccCT[ccI[wwhite[ii]]][ccI[bblack[ii]]] +=
						"<a href='javascript:OpenGame(" + ii + ")'>&#189;</a>";
					ccCT[ccI[bblack[ii]]][ccI[wwhite[ii]]] +=
						"<a href='javascript:OpenGame(" + ii + ")'>&#189;</a>";
				}
				ccC[ccI[wwhite[ii]]] += 1;
			...
			dd.writeln(
				"<table border=1 celpadding=0 cellspacing=0 width='100%'><tr><th>Rank</th><th>Name</th>"
			);
			for (kk = 0; kk < ccL; kk++) dd.writeln('<th>' + (kk + 1) + '</th>');
			dd.writeln('<th>Score</th></tr>');
			for (kk = 0; kk < ccL; kk++) {
				dd.writeln(
					'<tr><th nowrap>' + (kk + 1) + '</th><th nowrap>' + ccN[ccO[kk]] + '</th>'
				);
				for (ii = 0; ii < ccL; ii++)
					dd.writeln('<th nowrap>' + ccCT[ccO[kk]][ccO[ii]] + '&nbsp;</th>');
				dd.writeln(
					'<th nowrap>' +
						Math.round(10 * ccS[ccO[kk]]) / 10 +
						'/' +
						ccC[ccO[kk]] +
						'</th></tr>'
				);
...
function ScoreSheetHeader(tt) {
	...
	ss =
		"<div align='center'><table width=50% cellpadding=0 cellspacing=0><tr><td><table width=100% cellpadding=2 cellspacing=0>";
	ss +=
		'<tr><td colspan=9><small>event</small><br /><nobr>' +
		vv[0] +
		'</nobr></td><td colspan=5><small>date</small><br /><nobr>' +
		vv[2] +
		'</nobr></td></tr>';
	ss +=
		'<tr><td colspan=9><small>site</small><br /><nobr>' +
		vv[1] +
		"</nobr></td><td width='8%'><small>rnd</small><br /><nobr>" +
		vv[3] +
		"</nobr></td><td width='16%' colspan=2><small>score</small><br /><nobr>" +
		vv[4] +
		"</nobr></td><td width='16%' colspan=2><small>eco</small><br /><nobr>" +
		vv[7] +
		'</nobr></td></tr>';
	ss +=
		'<tr><td colspan=11><small>white</small><br /><nobr>' +
		vv[5] +
		'</nobr></td><td colspan=3><small>rating</small><br /><nobr>' +
		vv[8] +
		'</nobr></td></tr>';
	ss +=
		'<tr><td colspan=11><small>black</small><br /><nobr>' +
		vv[6] +
		'</nobr></td><td colspan=3><small>rating</small><br /><nobr>' +
		vv[9] +
		'</nobr></td></tr>';
	if (vv[10])
		ss += '<tr><td colspan=14><small>fen</small><br /><nobr>' + vv[10] + '</nobr></td></tr>';
	ss += '</table>';
	return ss;
}
...
function PrintPosition() {
	...
	if (document.BoardForm) {
		...
		if (parseInt(document.BoardForm.Position.value) > 0)
			tt += document.BoardForm.Position.value;
		else tt = document.BoardForm.Position.value;
	}
	ww = window.open('');

	var wwDocument = ww.document;

	wwDocument.open();
	wwDocument.writeln('<html><head><title>' + tt + "</title></head><body><div align='center'>");
	wwDocument.writeln(GetDiagram(pp, tt));

	if (Annotation[MoveCount]) {
		wwDocument.writeln(Annotation[MoveCount]);
	}

	wwDocument.writeln('</div></body></html>');
	wwDocument.close();
...
function SetDragDrop(bb) {
	if (document.BoardForm && document.BoardForm.DragDrop) document.BoardForm.DragDrop.checked = bb;
	if (document.layers) return;
	SetBoardClicked(-1);
	isDragDrop = bb;
	if (bb) {
...
function AnimateBoard(nn) {
	...
	if (nn % mm == 1) {
		if (isRotated)
			dragImg[pp % 3] =
				document.images[63 - dragPiece[pp + 2] - (7 - dragPiece[pp + 3]) * 8 + ImageOffset];
		else
			dragImg[pp % 3] =
				document.images[dragPiece[pp + 2] + (7 - dragPiece[pp + 3]) * 8 + ImageOffset];
		dragPiece[pp + 2] = dragImg[pp % 3].offsetLeft;
		dragPiece[pp + 3] = dragImg[pp % 3].offsetTop;
		if (isRotated)
			dragImg[pp % 3] =
				document.images[63 - dragPiece[pp + 0] - (7 - dragPiece[pp + 1]) * 8 + ImageOffset];
		else
			dragImg[pp % 3] =
				document.images[dragPiece[pp + 0] + (7 - dragPiece[pp + 1]) * 8 + ImageOffset];
	...
	if (nn % mm != 0) {
		...
		dragImg[pp % 3].style.left =
			Math.round(((nn % mm) * (dragPiece[pp + 2] - dragPiece[pp + 0])) / (mm - 1)) +
			dragImgBorder +
			'px';
		dragImg[pp % 3].style.top =
			Math.round(((nn % mm) * (dragPiece[pp + 3] - dragPiece[pp + 1])) / (mm - 1)) +
			dragImgBorder +
			'px';
